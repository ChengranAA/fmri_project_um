...
lower_end = 1
upper_end = 9
use_row = str(lower_end)+":"+str(upper_end)
counter_list = []
counter = 1
...
expName = 'LocalizerExperiment'  # from the Builder filename that created this script
expInfo = {
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/lcraaaa/Desktop/LocalizerExperiment/LocalizerExperiment_lastrun.py',
...
filename2 = "summarized_datafile"
thisExp2 = data.ExperimentHandler(name=expName, version='',
    extraInfo=expInfo, runtimeInfo=None,
    originPath='C:\\Users\\herrd\\Documents\\GitHub\\fmri_project_um\\TestRun\\data_summarized',
    savePickle=True, saveWideText=True,
...
# --- Initialize components for Routine "routine_0_75sec_Face_House" ---
...
WaitForScannerComponents = [key_resp, text_2, polygon]
...
while continueRoutine:
    ...
    if len (key_resp.keys) == 5:
                for x in key_resp.keys:
                    counter = counter+1
                    counter_list.append(counter)
                thisExp.addData('counter', counter_list)
                thisExp2.addData('condition', "waiting")
                thisExp2.addData('counter', counter_list)
                thisExp2.nextEntry()
                counter_list = []
...
blocks = data.TrialHandler(nReps=12.0, method='sequential', 
...
for thisBlock in blocks:
    ...
    trials = data.TrialHandler(nReps=1.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions(cond_file, selection=use_row),
    ...
    for thisTrial in trials:
        ...
        # --- Prepare to start Routine "routine_0_75sec_Face_House" ---
        continueRoutine = True
        routineForceEnded = False
        # update component parameters for each repeat
        image.setImage(stim_images)
        # Run 'Begin Routine' code from code_3
        store_picture1 = trials.thisTrial.get("stim_images")
        # keep track of which components have finished
        routine_0_75sec_Face_HouseComponents = [image, polygon_2]
        for thisComponent in routine_0_75sec_Face_HouseComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Run Routine "routine_0_75sec_Face_House" ---
        ...
        # --- Ending Routine "routine_0_75sec_Face_House" ---
        for thisComponent in routine_0_75sec_Face_HouseComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # Run 'End Routine' code from code_3
        thisExp2.addData('picture1', store_picture1)
        thisExp2.addData('counter', counter_list)
        thisExp2.nextEntry()
        ...
        routineForceEnded = False
        # update component parameters for each repeat
        image_2.setImage(stim_images2)
        key_resp_5.keys = []
        key_resp_5.rt = []
        _key_resp_5_allKeys = []
        # Run 'Begin Routine' code from code_6
        store_picture2 = trials.thisTrial.get("stim_images2")
        # keep track of which components have finished
        test3Components = [image_2, key_resp_5, polygon_3]
        ...
        while continueRoutine:
            ...
            if key_resp_5.status == STARTED and not waitOnFlip:
                theseKeys = key_resp_5.getKeys(keyList=['5'], waitRelease=False)
            ...
            if len (key_resp_5.keys) == 1:
                        counter = counter+1
                        counter_list.append(counter)
                        thisExp2.addData('picture2', store_picture2)
                        thisExp2.addData('counter', counter_list)
                        thisExp2.nextEntry()
                        thisExp.addData('counter', counter_list)
                        counter_list = []
    ...
    _key_resp_3_allKeys = []
    # keep track of which components have finished
    RestComponents = [key_resp_3, polygon_4]
    ...
    while continueRoutine:
        ...
        if key_resp_3.status == STARTED and not waitOnFlip:
            theseKeys = key_resp_3.getKeys(keyList=['5'], waitRelease=False)
        ...
        if len (key_resp_3.keys) == 8:
                    for x in key_resp_3.keys:
                        counter = counter+1
                        counter_list.append(counter)
                    thisExp.addData('counter', counter_list)
                    thisExp2.addData('condition', "resting")
                    thisExp2.addData('counter', counter_list)
                    thisExp2.nextEntry()
                    counter_list = []
                    continueRoutine = False
    ...
    store_rep = blocks.thisTrialN
    print(store_rep)
    if store_rep == 1:
        lower_end = lower_end+8
        upper_end = upper_end+8
        print("lowerend:"+str(lower_end)+"and"+"upperend:"+str(upper_end))
        use_row = str(lower_end)+":"+str(upper_end)
    # the Routine "Rest" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
    
# completed 12.0 repeats of 'blocks'
